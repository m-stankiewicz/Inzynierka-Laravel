                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <td>@{{ item.vat_id }}</td>
                                        <td>@{{ item.address }}</td>
                                        <td v-text="item.is_company == 1 ? '{{ trans('admin.customer.columns.company') }}' : '{{ trans('admin.customer.columns.individual') }}'"></td>
                                        <td>
                                            <div class="row no-gutters">
                                                <div class="col-auto">
                                                    <a class="btn btn-sm btn-spinner btn-info" :href="'customers/' + item.id + '/edit'" title="{{ trans('brackets/admin-ui::admin.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                </div>
                                                <form class="col" @submit.prevent="deleteItem('customers/' + item.id)">
